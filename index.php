...
function calculate($operand1, $operand2, $operation) { //四則計算
    switch ($operation) {
        case '+':
...
function percent_calculate($operand1, $operand2, $operation){
    switch ($operation) {
        case '+':
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($input) {
        ...
        case '%':
            if (isset($_SESSION['operation']) && isset($_SESSION['operand']) && !$isNewOperation){
                $firstOperand = (float)$_SESSION['operand'];
                $currentOperand = ((float)$currentDisplay / 100);
                $operation = $_SESSION['operation'];

                $result = percent_calculate($firstOperand, $currentOperand, $operation);
                $display = (string)$result;
